...
string cmd_motion_type = "Hold";    // flag for switching Robot Motion
...
int flag_trunk_DOB = 1;     // flag for switching ON/OFF Trunk DOB control
...
double trunk_p_gain = 80;
double trunk_d_gain = 2;
double trunk_pd_cutoff = 500;

double trunk_DOB_cutoff = 500;

double omega_n = 100;
double zeta = 5;
double k = 500;

double leg_p_gain = 1000;
...
void mycontroller(const mjModel* m, mjData* d)
{
    ...
    state_Model_Trunck.ctrl_input_RW_from_trunk = ctrl_Trunk.Feedforward_vel(&state_Model_Trunck, flag_trunk_ff);

    // Trunk PID Gain Setting
    ctrl_Trunk.pid_gain_pos(trunk_p_gain, trunk_d_gain, trunk_pd_cutoff, flag_trunk_pid);

    // Trunk PID control
    state_Model_Trunck.ctrl_input_RW_from_trunk = state_Model_Trunck.ctrl_input_RW_from_trunk + state_Model_Trunck.jacb_trunk_pos * ctrl_Trunk.PID_pos(&state_Model_Trunck);

    // Trunk DOB control
    state_Model_Trunck.ctrl_input_RW_from_trunk = state_Model_Trunck.ctrl_input_RW_from_trunk + ctrl_Trunk.DOB(&state_Model_Trunck, trunk_DOB_cutoff, flag_trunk_DOB);

    // RW input to each legs
    state_Model_FL.posRW_ref[0] = state_Model_Trunck.ctrl_input_RW_from_trunk[0];
    state_Model_FR.posRW_ref[0] = state_Model_Trunck.ctrl_input_RW_from_trunk[1];
    state_Model_RL.posRW_ref[0] = state_Model_Trunck.ctrl_input_RW_from_trunk[2];
    state_Model_RR.posRW_ref[0] = state_Model_Trunck.ctrl_input_RW_from_trunk[3];
