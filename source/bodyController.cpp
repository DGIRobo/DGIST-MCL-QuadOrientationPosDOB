...
void bodyController::pid_gain_vel(double kp, double kd, double cut_off_vel, int flag)
{
    if (flag == 1) {
        for (int i = 0; i < NDOF_TRUNK; i++)
        {
...
void bodyController::PID_vel(TrunkModel_* trunk_model)
{
    for (int i = 0; i < NDOF_TRUNK; i++)
    {
        error_vel[i] = trunk_model->vel_trunk_des[i] - trunk_model->vel_trunk_est[i]; //error is no vector
        error_old_vel[i] = trunk_model->vel_trunk_des_old[i] - trunk_model->vel_trunk_est_old[i];
