...
void init_save_data_trunk(FILE* fid)
{
    ...
    fprintf(fid, "y_des, alpha_des, beta_des, ");
    fprintf(fid, "y_est, alpha_est, beta_est, ");
    fprintf(fid, "y_act, alpha_act, beta_act");

    // Don't remove the newline
    fprintf(fid, "\n");
}

void save_data_trunk(const mjModel* m, mjData* d, TrunkModel_* trunk_model, FILE* fid)
{
    // This function is called once and is used to get the headers
    // Write name of the variable here (header)
    // comma(,) should be omitted in the last line.
    ...
    fprintf(fid, "%f, %f, %f, ", trunk_model->pos_trunk_des[0], trunk_model->pos_trunk_des[1], trunk_model->pos_trunk_des[2]);
    fprintf(fid, "%f, %f, %f, ", trunk_model->pos_trunk_est[0], trunk_model->pos_trunk_est[1], trunk_model->pos_trunk_est[2]);
    fprintf(fid, "%f, %f, %f", trunk_model->pos_trunk_act[2], trunk_model->pos_trunk_act[5], trunk_model->pos_trunk_act[4]);
